...
# https://about.cmrad.com/articles/the-ultimate-guide-to-preprocessing-medical-images-techniques-tools-and-best-practices-for-enhanced-diagnosis
    ...
    def original_vs_preprocessed_plot(self,original, processed, title1= "Original Image", title2= "Preprocessed Image"):
    ...
    def remove_background_otsu(self,image): # uses otsu thresholding to remove the background.
        _, otsu_threshold = cv2.threshold(image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        return otsu_threshold

    def plot_multiple_thresholds(self,image, ground_truth, thresholds):
        ...
        for idx, threshold in enumerate(thresholds, start=2):
            bg_removed = self.thresholding(image, threshold)
    ...
    def resample_image(self,image, target_shape): # resampling = changing the pixel size of an image without altering its resolution.
    ...
    def translate_image(self,image, tx=30, ty=20):
    ...
    def normalize_intensity(self,image, min_percentile= 0.5, max_percentile=99.5): # normalize the image intensities to ensure consistency across the dataset.
    ...
    def adapted_thresholding(self,image): # uses adapted thresholding to remove the background.
    ...
    def apply_box_filter(self,image):
        return uniform_filter(image, size=5)

    def apply_a_trimmed_mean_filter(self,image, alpha, kernel_size):
        return generic_filter(image, lambda x: trim_mean(x, proportiontocut=alpha), size= kernel_size)

    def apply_gaussian_filter(self,image, sigma= 2):
    ...
    def find_ground_truth(self,filename, image): # this function can be used to find the ground truth label of an image for the DUKE
        # dataset.
        ground_truth_path = os.path.join(self.ground_truth_directory, filename)
...
input_directory = preprocessor.input_directory    
ground_truth_directory = preprocessor.ground_truth_directory

for filename in os.listdir(input_directory):
    if filename.lower().endswith(".npz"):  # RETOUCH dataset files
        npz_path = os.path.join(input_directory, filename)
        try:
            ground_truth = preprocessor.handle_npz_images(npz_path, filename)
    ...
    bg_removed = preprocessor.thresholding(image, 0.25)
    
    # apply morphological closing
    bg_closed = morphology.closing(bg_removed, morphology.square(10))
    ...
    plt.subplot(1, 4, 3)
    plt.imshow(bg_closed, cmap='gray')
    plt.title("Morphological Closing")

    # Ground truth to overlay
    plt.subplot(1, 4, 4)
    plt.imshow(image, cmap='gray')
    plt.contour(ground_truth, colors='red', linewidths=1)
